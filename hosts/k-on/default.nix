...
in
{
  ...
  boot = {
    # systemd passes the swapfile resume information to initrd through the
    # HibernateLocation EFI variable, so resumeDevice and resume_offset do not
    # need to be set manually. See systemd-hibernate-resume-generator(8):
    # https://www.freedesktop.org/software/systemd/man/latest/systemd-hibernate-resume-generator.html
    # resumeDevice = "/dev/mapper/pool-root"; # Device containing the swapfile
    kernel.sysctl = {
  ...
  programs.rvc = {
    enable = true;
    package = inputs.rvc.packages.x86_64-linux.cuda-with-models;
    chinese.enable = true;
